...
ALLEGRO_BITMAP *backGroundFimTempo = NULL;
...
ALLEGRO_FONT *textPlayer = NULL;
ALLEGRO_FONT *ultimaChance = NULL;
...
int posXBtCenarioEspaco = 1503;
int posXBtCenarioCidade = 1515;
...
void resetJogo(){
    ...
    posXBackGround=0;
    posXNave = posXNave;
    posXObst = (rand()% TELA_LARGURA*1.5) + 3000;
    posXObst2 = (rand()% TELA_LARGURA*1.5) + 3000;
    posXObst3 = (rand()% TELA_LARGURA*1.5) + 3000;
    posXObst4 = (rand()% TELA_LARGURA*1.5) + 3000;
    posXExploNave = 4000;
    posXExploObst = 4500;
    posXExploObst2 = 4500;
    posXExploObst3 = 4500;
    ...
    //Declara e inicializa Variaveis com a posição Y dos objetos
    posYBackGround=0;
    posYObst = (rand()% 250) + al_get_bitmap_height(obst4);
    posYObst2 = (rand()% 500) + 270 ;
    posYObst3 = (rand()% 790) + 540 ;
    posYObst4 = (rand()% (TELA_ALTURA-al_get_bitmap_height(obst4))) + 810 ;
    posYExploNave = 4000;
    posYVilao = 100;
    posYVida1 = 10;
    posYVida2 = 10;
...
void loadBitmap(){
    ...
    backGroundFimTempo =    al_load_bitmap("img/back/1920/telaFimTempo/TELA_FimTempo.jpg");
    ...
    nave[1][1] =            al_load_bitmap("img/naves/naveUmEstadoUm.png");
    nave[1][2] =            al_load_bitmap("img/naves/naveUmEstadoDois.png");
    nave[2][0] =            al_load_bitmap("img/naves/naveDoisEstadoZero.png");
    nave[2][1] =            al_load_bitmap("img/naves/naveDoisEstadoUm.png");
    nave[2][2] =            al_load_bitmap("img/naves/naveDoisEstadoDois.png");
    ...
    obst4 =                 al_load_bitmap("img/asteroide.png");
    explo =                 al_load_bitmap("img/explo.png");
    fogo =                  al_load_bitmap("img/fogo_small.png");
    vilao =                 al_load_bitmap("img/planetas/planetaUm.png");
    ...
    textPlayer =            al_load_font("fonte/alarm_clock/alarm_clock.ttf", 90, ALLEGRO_TTF_NO_KERNING);
    textScoreJogo =         al_load_font("fonte/alarm_clock/alarm_clock.ttf", 80, ALLEGRO_TTF_NO_KERNING);
    ultimaChance =          al_load_font("fonte/alarm_clock/alarm_clock.ttf", 80, ALLEGRO_TTF_NO_KERNING);
    tiro =                  al_load_bitmap("img/tiroDois.png");
    tiro1 =                 al_load_bitmap("img/tiroDois.png");
    tiro2 =                 al_load_bitmap("img/tiroDois.png");
    tiro3 =                 al_load_bitmap("img/tiroDois.png");
...
void telaEscolha(){//----------------------------------------------------FUNCAO RESPONSALVEL PELO MENU
     ...
     al_draw_textf(textPlayer, al_map_rgb(255, 255, 0), 980, 631, ALLEGRO_ALIGN_CENTER, "%s",nomePlayer);
...
void telaJogo(){//----------------------------------------------------FUNCAO RESPONSALVEL PELO JOGO
    ...
    if(posColisao<120){//---------------------------------------------PAUSA APOS OCORRENCIA DE COLISAO(JOGO INATIVO)
        ...
        posXVilao -= velVilao;
        if (posXVilao<=-(al_get_bitmap_width(vilao)+10)){
            posYVilao = (rand()% (TELA_ALTURA-al_get_bitmap_height(vilao))) + 0;
            posXVilao = (rand()% TELA_LARGURA*1.5) + 2000;
        }
        ...
        angObst4+=0.0349066;
        posXObst4 -= (velObst4+velNave);
        if(posXObst4<-50){
            posXObst4 = (rand()% TELA_LARGURA*1.5) + 2000;
            posYObst4 = (rand()% 1080) + 810;
            velObst4 = rand()% 7 + 1;
            score ++;
            somaScore ++;
            printf("SomaScore:%i\n", somaScore);
        ...
        if(tempTiro == 45){
            ...
            if (posXTiro>= TELA_LARGURA+100){
                posXTiro = posXNave;
                posYTiro = posYNave-30;
            }
        }
        posXTiro1 += velTiro1;
        if(tempTiro == 90){
            if (posXTiro1>= TELA_LARGURA+100){
                posXTiro1 = posXNave;
                posYTiro1 = posYNave-30;
        ...
        posXTiro2 += velTiro2;
        if(tempTiro == 135){
            if (posXTiro2>= TELA_LARGURA+100){
                posXTiro2 = posXNave;
                posYTiro2 = posYNave-30;
        ...
        posXTiro3 += velTiro3;
        if(tempTiro == 180){
            if (posXTiro3>= TELA_LARGURA+100){
                posXTiro3 = posXNave;
                posYTiro3 = posYNave-30;
            }
        ...
        if (colisao(nave[tipoNave][estadoNave],posXNave,posYNave,obst3,posXObst3,posYObst3)){
         ...
         if (colisao(nave[tipoNave][estadoNave],posXNave,posYNave,obst4,posXObst4,posYObst4)){
            tempExploNave = 0;
            posXExploNave = posXNave;
            posYExploNave = posYNave-(al_get_bitmap_height(explo)-al_get_bitmap_height(nave[tipoNave][estadoNave]))/2;
            posYNave = 6000;
            posXNave = 6000;
            posXObst4 = (rand()% TELA_LARGURA*1.5) + 2000;
            posYObst4 = (rand()% TELA_ALTURA-al_get_bitmap_height(obst4)) + 700;
            posXplusTime = (rand()% TELA_LARGURA*1.5) + 2000;
            posXplusScore = -1000;
            vidas --;
            posYTiro = 10000;
        ...
        if (colisao(tiro,posXTiro,posYTiro,obst,posXObst,posYObst)){
            posXObst = 5000;
            posYTiro = 10000;

        }
        if (colisao(tiro,posXTiro,posYTiro,obst,posXObst2,posYObst2)){
            posXObst2 = 5000;
            posYTiro = 10000;

        }
        if (colisao(tiro,posXTiro,posYTiro,obst3,posXObst3,posYObst3)){
            posXObst3 = 5000;
            posYTiro = 10000;
        }
        if (colisao(tiro,posXTiro,posYTiro,obst4,posXObst4,posYObst4)){
            posXObst4 = 5000;
            posYTiro = 10000;
        }
        //--------------------------------------------------
        if (colisao(tiro1,posXTiro1,posYTiro1,obst,posXObst,posYObst)){
            posXObst = 5000;
            posYTiro1 = 10000;
        }
        if (colisao(tiro1,posXTiro1,posYTiro1,obst,posXObst2,posYObst2)){
            posXObst2 = 5000;
            posYTiro1 = 10000;        }
        if (colisao(tiro1,posXTiro1,posYTiro1,obst3,posXObst3,posYObst3)){
            posXObst3 = 5000;
            posYTiro1 = 10000;
        }
        if (colisao(tiro1,posXTiro1,posYTiro1,obst4,posXObst4,posYObst4)){
            posXObst4 = 5000;
        }
        //-------------------------------------------------------------
        if (colisao(tiro2,posXTiro2,posYTiro2,obst,posXObst,posYObst)){
                posXObst = 5000;
                posYTiro2 = 10000;
        }
        if (colisao(tiro2,posXTiro2,posYTiro2,obst,posXObst2,posYObst2)){
            posXObst2 = 5000;
            posYTiro2 = 10000;        }
        if (colisao(tiro2,posXTiro2,posYTiro2,obst3,posXObst3,posYObst3)){
            posXObst3 = 5000;
            posYTiro2 = 10000;
        }
        if (colisao(tiro2,posXTiro2,posYTiro2,obst4,posXObst4,posYObst4)){
            posXObst4 = 5000;
            posYTiro2 = 10000;
        }
        //---------------------------------------------------------------
        if (colisao(tiro3,posXTiro3,posYTiro3,obst,posXObst,posYObst)){
                posXObst = 5000;
                posYTiro3 = 10000;
        }
        if (colisao(tiro3,posXTiro3,posYTiro3,obst,posXObst2,posYObst2)){
            posXObst2 = 5000;
            posYTiro3 = 10000;        }
        if (colisao(tiro3,posXTiro3,posYTiro3,obst3,posXObst3,posYObst3)){
            posXObst3 = 5000;
            posYTiro3 = 10000;
        }
        if (colisao(tiro3,posXTiro3,posYTiro3,obst4,posXObst4,posYObst4)){
            posXObst4 = 5000;
            posYTiro3 = 10000;
        }
...
void drawTelaJogo (){
    ...
    al_draw_bitmap(tiro2, posXTiro2, posYTiro2, 0);
    al_draw_bitmap(tiro3, posXTiro3, posYTiro3, 0);
    ...
    al_draw_bitmap(obst4, posXObst4, posYObst4, 0);
    ...
    if (vidas == 0){
        al_draw_textf(ultimaChance, al_map_rgb(rTime, gTime, bTime), posXUltimaChance-150, posYUltimaChance, ALLEGRO_ALIGN_LEFT, "!!ULTIMA CHANCE!!");
    }
}

void telaFimTempo(){
    //Monta a telaFimTempo
     al_draw_bitmap(backGroundFimTempo, 0, 0, 0);
     tempTelaFimTempo++;
